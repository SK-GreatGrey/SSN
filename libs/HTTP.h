...
typedef enum {
  ...
  Malformed = 2,
  OutOfMemory = 3,
  URLTooLong = 4 // Originally existed because the URL was fixed size in the
                 // struct, but kept for extra safety
} InvalidReason;
...
int HTTPRequest_add_header(HTTPRequest *response, const char *name,
                           const char *value);
