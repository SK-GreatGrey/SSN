...
int HTTPRequest_add_header(HTTPRequest *request, const char *name,
                           const char *value) {
...
const char *HTTPRequest_tostring(HTTPRequest *request) {
  ...
  int messageSize =
      2 + strlen(method) + strlen(HTTP_VERSION) + strlen(request->URL);
  if (request->headers != NULL) {
...
int HTTPResponse_add_header(HTTPResponse *response, const char *name,
                            const char *value) {
...
const char *HTTPResponse_tostring(HTTPResponse *response) {
  ...
  char *status = malloc(messageSize);
  // write first line
  int curPos = snprintf(status, messageSize, "%s %d %s", HTTP_VERSION,
